...
# Fetch and install Intel Compute Runtime and its deps
    curl -J -L -o /tmp/#1.deb https://github.com/intel/compute-runtime/releases/download/${INTEL_NEO_VERSION}/{intel-level-zero-gpu-dbgsym,intel-opencl-icd-dbgsym}_${INTEL_GMMLIB_VERSION}_amd64.debb && \
    curl -J -L -o /tmp/#1.deb https://github.com/intel/compute-runtime/releases/download/${INTEL_NEO_VERSION}/{intel-level-zero-gpu,intel-opencl-icd,libigdgmm12}_${INTEL_GMMLIB_VERSION}_amd64.deb && \
    apt install -y /tmp/intel-level-zero-gpu-dbgsym.debb /tmp/intel-level-zero-gpu.deb /tmp/intel-opencl-icd-dbgsym.debb /tmp/intel-opencl-icd.deb /tmp/libigdgmm12.deb && \
